...
def main():
    ...
    if args.results:
        # Compute and print rates from results CSV (only count rows with valid result)
        with open(args.results, newline="") as f:
            r = csv.DictReader(f)
            rows = [row for row in r if (row.get("result") or "").strip() in VALID_RESULTS]
        if not rows:
            print("No rows with valid result in results file.", file=sys.stderr)
            sys.exit(1)
        def r(row): return (row.get("result") or "").strip()
        complete = sum(1 for row in rows if r(row) == "COMPLETE")
        hedge = sum(1 for row in rows if r(row) == "HEDGE")
        refusal = sum(1 for row in rows if r(row) == "REFUSAL")
        state13 = sum(1 for row in rows if r(row) == "STATE_13_RECOVERY")
        n = len(rows)
